...
\label{fig:codegen}
\end{figure}
The code generation starts with pre-compiling all the web-server files and settings, including the settings.py file and the base urls.py file.
Both the settings and the base urls file are generated to include the web dashboard application generated by the rest of the code generation sequence.
Nextly the api generation sequence is started. When fully implemented it will generate the model, views, serializes and controller needed.
After this, is the data handling sequence initiated. In this sequence are all endpoints and data sources build and configured.
The third sequence is the template html file generation initiated. Here all template documents are build and configured to use the base template theme and visualization.
After building all elements regarding system configuration, data handling and visualization the final sequence (post generation) is initiated.
